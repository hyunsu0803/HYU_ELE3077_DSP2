%%% 2st order : notch filter (r = 0.95) %%%
sgtitle('2st order : notch filter (r = 0.95)')
...
w0  = pi/6;
z = exp(j*0);
G1 = (1 -2*r*cos(w0)/z + (r*r)/(z*z)) / (1 -2*cos(w0)/z + 1/(z*z));
num = [1, -2*cos(w0), 1];
...
w0  = pi/3;
z = exp(j*0);
G2 = (1 -2*r*cos(w0)/z + (r*r)/(z*z)) / (1 -2*cos(w0)/z + 1/(z*z));
num = [1, -2*cos(w0), 1];
den = [1, -2*r*cos(w0), r*r];
